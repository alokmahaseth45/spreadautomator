...
for listpath_new in pdf_links_list:
	print listpath_new
	if os.path.isfile(listpath_new):
		...
		bat_text_profit_Bat =  'sqlcmd -E -S localhost -d Annual_Reports -Q'+' '+ '"Exec SP_Read_Conversion_Log'+' '+"'"+Request_id+"'"+'"'
		...
		if "Page not break"  in return_output:
			...
			with open(log_request ,'a') as log:
				...
				log.write('End Date_Time to insert text in sql-->'+end_date+'\n')
			...
			end_date  = time.strftime("%x %H:%M:%S")
			with open(log_request ,'a') as log:
				log.write('Start Date_Time to insert excel data in sql-->'+start_date+'\n')
				log.write('End Date_Time to insert excel data in sql-->'+end_date+'\n')
								   # 'sqlcmd -E -S localhost -d Annual_Reports_Raw -Q'+' '+ '"Exec Annual_Reports_Raw.dbo.read_text_files'+' '+ "'"+Request_id+ "'"+'"'
			bat_text_profit_Bat =  'sqlcmd -E -S localhost -d Annual_Reports -Q'+' '+ '"Exec SP_Read_Conversion_Log'+' '+"'"+Request_id+"'"+'"'
			with open(call_log_bat, "w") as text_file:
				text_file.write(str(bat_text_profit_Bat)+'\n')
			if os.path.isfile(call_log_bat):
