...
for listpath_new in pdf_links_list:
	...
	if os.path.isfile(listpath_new):
		...
		if "Page not break"  in return_output:
			...
			with open(log_request ,'a') as log:
				log.write('Start Date_Time to insert single excel data in sql-->'+start_date+'\n')
			...
			with open(log_request ,'a') as log:
				...
				log.write('End Date_Time to insert single excel data in sql-->'+end_date+'\n')
