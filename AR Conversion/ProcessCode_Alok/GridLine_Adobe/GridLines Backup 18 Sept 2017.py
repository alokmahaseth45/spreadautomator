...
def Convert_png_and_pdf(pdf_name):
	...
	for y in range(h):
		...
		elif x_sum > 0 and start_flag == 1 and y == y_white_last + 1 and y > y_black_last + 1 and y_black_last > y_black_last_line and y-y_black_last_line > 5 and x_dot_count_last <= 5:
				...
				if temp[y][x] == 255 and temp[y-1][x] == 255 and temp[y][x+1] == 255 and temp[y][x+2] == 255 and temp[y][x-1] == 255 and temp[y][x-2] == 255 and x < min(line_list[y])-2 and y_white_start in white_list_0 and y not in no_line_list:
					...
					if (temp[y+1][x] < 255 or y+1 in no_line_list) and y - y_white_start > 100:
						...
						if draw_line_flag == 1:
							...
							if x == 323:
								print 'existing_yline1',existing_yline1,existing_yline2
							...
							print x,'line',x_new,y_white_start,y_end-1
							current_line = []
							for c in range(y_white_start+1,y_end):
				...
				for x_a in reversed(sorted(line_dict_x_list)):
					...
					if x_a < x and line_dict[x]:
						...
						for current_line_a in line_dict[x_a]:
							...
							# if x == 323:
							# 	print x_a,subset_flag,'overlap_len',overlap_len,max(current_line),min(current_line),float(overlap_len/float(max(current_line)-min(current_line))),black_found
							if (subset_flag == 1 or float(overlap_len/float(max(current_line)-min(current_line))) > 0.7) and black_found < 14 and subset_flag != 2:
								subset_flag == 1
								if current_line in line_dict[x]:
									line_dict[x].remove(current_line)
								print 'deleted x',x
								col_del_flag = 1
								...
								subset_flag = 0
							# if x == 323:
							# 	print x_a,subset_flag,'overlap_len2',overlap_len,max(current_line_a),min(current_line_a),float(overlap_len/float(max(current_line_a)-min(current_line_a))),black_found
							# if subset_flag == 2 and black_found < 20:
							if (subset_flag == 2 or float(overlap_len/float(max(current_line_a)-min(current_line_a))) > 0.7) and black_found < 14 and subset_flag != 1:
	...
	print 'Deleting x'
	x_line_list_for_loop = x_line_list
	# print x_line_list_for_loop
	row_del_flag = 1
	while row_del_flag == 1:
		row_del_flag = 0
		for y in x_line_list_for_loop:
			# print y
			if str(line_dict.values()).replace('[',',').replace(']',',').replace(' ','').count(','+str(y)+',') <= 2:
				print 'removing x line',y
				x_line_list.remove(y)
				row_del_flag = 1
