	...
	if os.path.isfile(listpath_new):
		...
		try:
			...
			with open(log_request ,'a') as log:
				log.write('Start Date of collecting New.txt File-->'+str(start_date)+'\n')
			# pdb.set_trace()
			status = 'Start External Collecting New Files'
			Process_Head = 'Collect New Req'
			Process_Name = 'Collect New Req'
			Process_Time = '0'
			Current_Status = 'Start External Collecting New Files'
			...
			with open(log_request ,'a') as log:
				...
				log.write('End Date of collecting New.txt File-->'+str(end_date)+'\n')
			...
			Process_Name = 'Collect New Req'
			Process_Time = '0'
			Current_Status = 'End External Collecting New Files'
			...
			end_date = datetime.now()
			with open(log_request ,'a') as log:
				log.write('Start Date_Time of breaking pdf-->'+str(start_date)+'\n')
				log.write('End Date_Time of breaking pdf-->'+str(end_date)+'\n')
			...
			if "Page not break"  in return_output:
				...
				start_date = datetime.now()
				with open(log_request ,'a') as log:
					log.write('Start Date_Time of collecting single pdf files-->'+str(start_date)+'\n')
				status = 'Start External Collecting Single Pdf Files'

				Process_Head = 'Collect New Req'
				Process_Name = 'Break Pdf Page'
				Process_Time = '0'
				Current_Status = 'Start External Collecting Single Pdf Files'
				# Time_Stamp = str(end_date)
				# cal_end = end_date - start_date
				# Process_Time =  str(cal_end.total_seconds())
				# call_log(Request_id,status,call_log_bat)
				...
				with open(log_request ,'a') as log:
					...
					log.write('End Date_Time of collecting single pdf files-->'+str(end_date)+'\n')
				status = 'End External Collecting Single Pdf Files'

				Process_Head = 'Collect New Req'
				Process_Name = 'Break Pdf Page'
				Process_Time = '0'
				Current_Status = 'End External Collecting Single Pdf Files'
				Time_Stamp = str(end_date)
				cal_end = end_date - start_date
				Process_Time =  str(cal_end.total_seconds())
				# call_log(Request_id,status,call_log_bat)
