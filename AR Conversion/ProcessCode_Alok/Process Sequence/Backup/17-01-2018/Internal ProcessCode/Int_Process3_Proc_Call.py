...
while 1 == 1:
	...
	for listpath_new in New_Request_List:
		...
		try:
			...
			read_m_bat = "D:\AR Conversion\ProcessCode_Alok\Process Sequence\Internal ProcessCode\DB_Insert MT Batch.bat"
		...
		bat_filesave1 = 'D:\AR Conversion\Request_ID\\'+str(Request_id)+'_1.bat'
		...
		if "Internal_1" in Internal_process:
			...
			Proc_log = 'D:\AR Conversion\ProcessCode_Alok\Proc log\\'+'SPV_AR_Process_2_'+Request_id+'_'+log_time_var+'.txt'
			# sqlcmd -E -S localhost -d Annual_Reports -Q "Exec Annual_Reports.dbo.Update_OverviewNewSite_OnClick 'U24236MH1987PTC044813'">
			bat_text =  'sqlcmd -E -S localhost -d Annual_Reports -Q'+' '+ '"Exec Annual_Reports.dbo.SPV_AR_Process_2'+' '+ "'"+Request_id+ "'"+' "'+'>>'+'"'+Proc_log+'"'
			with open(bat_filesave, "w") as text_file:
				text_file.write(str(bat_text)+'\n')
				text_file.write("exit"+'\n')

			with open(bat_filesave1, "w") as text_file:
				text_file.write(str(bat_text)+'\n')
				text_file.write("exit"+'\n')
			# pdb.set_trace()

			
			# sqlcmd -E -S localhost -d Annual_Reports -Q "Exec SPV_AR_Process_2 'ddd29f22-6669-47e3-a180-bff5331aa9bf' ">>"D:\AR Conversion\ProcessCode_Alok\GridLine_Adobe\Request_Id_Path\log\SPV_AR_Process_1ddd29f22-6669-47e3-a180-bff5331aa9bf@%date:/=-%_%time::=^%.txt"

		if "Internal_2" in Internal_process:
			if os.path.isfile(bat_filesave):
				os.remove(bat_filesave)
			Proc_log = 'D:\AR Conversion\ProcessCode_Alok\Proc log\\'+'SPV_Process_Internal_2_'+Request_id+'_'+log_time_var+'.txt'

			# sqlcmd -E -S localhost -d Annual_Reports -Q "Exec Annual_Reports.dbo.Update_OverviewNewSite_OnClick 'U24236MH1987PTC044813'">
			bat_text =  'sqlcmd -E -S localhost -d Annual_Reports -Q'+' '+ '"Exec Annual_Reports.dbo.SPV_Process_Internal_2'+' '+ "'"+Request_id+ "'"+' "'+'>>'+'"'+Proc_log+'"'
			with open(bat_filesave, "w") as text_file:
				text_file.write(str(bat_text)+'\n')
				text_file.write("exit"+'\n')

		if "Re_Run" in Internal_process:
			# print "-------------------"
			if os.path.isfile(bat_filesave):
				os.remove(bat_filesave)
			Proc_log = 'D:\AR Conversion\ProcessCode_Alok\Proc log\\'+'SPV_AR_Process_Internal_P1ReRun_'+Request_id+'_'+log_time_var+'.txt'
			bat_text =  'sqlcmd -E -S localhost -d Annual_Reports -Q'+' '+ '"Exec Annual_Reports.dbo.SPV_AR_Process_Internal_P1ReRun'+' '+ "'"+Request_id+ "'"+' "'+'>>'+'"'+Proc_log+'"'
			with open(bat_filesave, "w") as text_file:
				text_file.write(str(bat_text)+'\n')
				text_file.write("exit"+'\n')
