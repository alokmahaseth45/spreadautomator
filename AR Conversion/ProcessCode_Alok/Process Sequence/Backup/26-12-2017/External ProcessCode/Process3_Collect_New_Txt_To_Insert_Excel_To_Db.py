...
while 1 == 1:
	...
	for listpath_new in New_Request_List:
		...
		try:
			...
			# read_m_bat = "D:\AR Conversion\ProcessCode_Alok\Process Sequence\External ProcessCode\DB_Insert MT Batch.bat"
			# multi_bat_path = r"D:\AR Conversion\ProcessCode_Alok\Process Sequence\External ProcessCode\multi_bat_file.bat"
			db_list_call_bat = r"D:\AR Conversion\ProcessCode_Alok\Process Sequence\External ProcessCode\db_list_call_bat.bat"

			if os.path.isfile(db_list_call_bat):
				os.remove(db_list_call_bat)

			db_call = 'call '+'"D:\AR Conversion\ProcessCode_Alok\Process Sequence\External ProcessCode\open_And_Read_Excel_Omni.py" '+'"'+listpath+'"'
			with open(db_list_call_bat,'w') as wr:
				inp = wr.write(db_call+'\n')
				
			

			if os.path.isfile(db_list_call_bat):
				Function_bat_run(db_list_call_bat)

			converted_flag = 0
				
		except Exception,e:
			print "Error in inserting excel to db",str(e)
			converted_flag = 1
