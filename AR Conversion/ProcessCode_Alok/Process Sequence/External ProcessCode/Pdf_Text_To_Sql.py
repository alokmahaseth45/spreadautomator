...
def Insert_To_Sql(pdf_links_list,Request_id):

	insert_to_sql= r'D:\AR Conversion\ProcessCode_Alok\insert_text_to_sql.bat'

	table_name = 'Text_Data_'+Request_id
	bat_text_profit_Bat =  'sqlcmd -E -S localhost -d Annual_Reports -Q'+' '+ '"Exec Create_Text_Table'+' '+"'"+table_name+"'"+'"'
	with open(insert_to_sql, "w") as text_file:
		text_file.write(str(bat_text_profit_Bat)+'\n')
	if os.path.isfile(insert_to_sql):
		Function_bat_run(insert_to_sql)
	...
	for pdf_link in pdf_links_list:
		...
		if os.path.isfile(pdf_link) and os.path.isfile(pdf_text):
			...
			cursor.execute("INSERT INTO dbo.["+table_name+"] (RequestID,Pdf_Path,Pdf_Page_No,Text_Path,Text_Content,Content_Length) VALUES (?, ?,?,?,?,?)", (company_Id,pdf_link,page_no,pdf_text,content,content_len)) ###### Query execute data insert   #########
			connection.commit()
