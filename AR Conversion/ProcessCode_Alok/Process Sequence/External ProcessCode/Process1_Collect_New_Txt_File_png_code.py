...
def break_pdf_page(pdf_link):
	try:
		...
		# pdb.set_trace()
		pdf_link_xxxx = pdf_link+'xxxx'
		# print "pdf folder----------: ",pdf_link_xxxx
		pdf_folder = pdf_link_xxxx.split('\\')
		pdf_name = pdf_folder[-1]
		print "pdf name is : ",pdf_name
		print "pdf link is : ",pdf_link
		print "pdf folder is : ",pdf_folder

		# pdb.set_trace()
		pdf_single_filesave = pdf_link_xxxx.replace(pdf_name,'')
		...
		for page_no in range(0,total_page):
			...
			pdf_sub_files = pdf_name.replace('.pdfxxxx','_'+str(i+1)+'.pdf')
	...
	for listpath_new in New_Request_List:
		...
		if os.path.isfile(listpath_new):
			...
			try:
				...
				pdf_path_xxxx = pdf_path+'xxxx'
				# pdb.set_trace()

				# "D:\Test\test.jpg"

				if '.jpg' in pdf_path:
					jpg_to_pdf_path = pdf_path_xxxx.replace('.jpgxxxx','.pdf')
					jpg_to_pdf_bat = 'Call "D:\AR Conversion\ProcessCode_Alok\img2pdf\img2pdf.exe" "'+pdf_path+'" "'+jpg_to_pdf_path+'"'
					print jpg_to_pdf_bat

					# exit()
					# while not os.path.isfile(jpg_to_pdf_path):
					if not os.path.isfile(jpg_to_pdf_path):

						os.system(jpg_to_pdf_bat)
						time.sleep(2)

					pdf_path = jpg_to_pdf_path
					pdf_name = pdf_name+'xxxx'
					pdf_name = pdf_name.replace('.jpgxxxx','.pdf')
					print pdf_name
				...
				try:
					var_open = open(pdf_path, "rb")
					inputpdf_open = PdfFileReader(var_open)
					if inputpdf_open.isEncrypted:
						decrypt_flag = 1
					var_open.close()
				except Exception,e:
					print str(e)
				...
				if decrypt_flag == 1:
					decrypt_path = pdf_path_xxxx.replace('.pdfxxxx','_decrypt.pdf')
					call_decrypt = 'Call "D:\AR Conversion\ProcessCode_Alok\qpdf-7.0.0\\bin\qpdf.exe" --decrypt "'+pdf_path+'" "'+decrypt_path+'"'
					print call_decrypt

					# exit()
					while not os.path.isfile(decrypt_path):
						os.system(call_decrypt)
					os.remove(pdf_path)
					os.rename(decrypt_path,pdf_path)
				...
				if "Page not break"  in return_output:
					...
					print pdf_single_filesave
					pdf_name_without_extention = pdf_name+'xxxx'
					pdf_name_without_extention = pdf_name_without_extention.replace('.pdfxxxx','')
					print pdf_name_without_extention



					#-------- Start To Get Single Pdf Files   -----------
					pdf_files = glob.glob(pdf_single_filesave+'/'+pdf_name_without_extention+'*'+'.pdf')
					...
					page_count_bat = "D:\AR Conversion\ProcessCode_Alok\Process Sequence\External ProcessCode\page_count_bat.bat"
					if os.path.isfile(page_count_bat):
						os.remove(page_count_bat)
						
					page_count_text =  'sqlcmd -E -S localhost -d Annual_Reports -Q'+' '+ '"Exec Annual_Reports.dbo.SPV_HealthCheckUpdate_by_Py'+' '+ "'"+Request_id+ "'"+','+ "'"+str(page_count)+ "'"+' "'

					with open(page_count_bat, "w") as text_file:
						text_file.write(str(page_count_text)+'\n')
							# text_file.write("exit"+'\n')


					if os.path.isfile(page_count_bat):
						Function_bat_run(page_count_bat)
					# SPV_HealthCheckUpdate_by_Py
