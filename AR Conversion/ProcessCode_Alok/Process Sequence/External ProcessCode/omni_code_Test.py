...
# listpath_new = str(sys.argv[1])
# omni_savedir = str(sys.argv[2])
# type_p = str(sys.argv[3])


listpath_new = r"D:\AR Conversion\ProcessCode_Alok\Request\External Omni Request\28ecbf54-88db-4f29-9ac3-44840bc1a35b_Num3-5_Pending.txt"
omni_savedir = r'D:\Omni Servers\13-229-215-84'
type_p = 'External'
	...
	try:
		...
		if os.path.isfile(listpath_new):
			...
			for pdf in pdf_list:
				...
				#-------- Start To Find Single Pdf Name   -----------
				pdf_path_xxxx = pdf+'xxxx'
				pdf_path  =  pdf_path_xxxx.split('\\')
				pdf_name = pdf_path[-1]
				#-------- End To Find Single Pdf Name  -----------
				...
				pdf_xlsx= pdf_path_xxxx.replace('.pdfxxxx',pdf_xlsx_parameter)
				pdf_txt = pdf_path_xxxx.replace('Annual Reports','Annual Reports Converted Omni')
				pdf_txt= pdf_txt.replace('.pdfxxxx',pdf_txt_parameter)
				pdf_xlsx_main = pdf_xlsx= pdf_path_xxxx.replace('.pdfxxxx','.xlsx')

				#---------End To Create Excel And Text Files In Main Folder------------

				#-------- Start "BSE Annual Reports Converted" xlsx_dir and xlsx_file  -----------
				xlsx_dir = pdf_path_xxxx.replace(pdf_name,'').replace('Annual Reports','Annual Reports Converted Omni')
				xlsx_filesave = xlsx_dir+pdf_name
				xlsx_filesave_main = xlsx_filesave.replace('.pdfxxxx','.xlsx')
				xlsx_filesave = xlsx_filesave.replace('.pdfxxxx',pdf_xlsx_parameter)
				print xlsx_filesave
				...
				pdf_xlsx_omni= omni_pdf_filesave.replace('.pdfxxxx',pdf_xlsx_parameter)
				pdf_txt_omni= omni_pdf_filesave.replace('.pdfxxxx',pdf_txt_parameter)
				pdf_xlsx_main_omni= omni_pdf_filesave.replace('.pdfxxxx','.xlsx')
				s_omni_pdf_filesave = omni_pdf_filesave.replace('.pdfxxxx','_0001.pdf')
				omni_pdf_filesave = omni_pdf_filesave.replace('.pdfxxxx','.pdf')
				...
				while main_flag ==1:
					...
					if (os.path.isfile(omni_pdf_filesave) and os.path.isfile(s_omni_pdf_filesave)) and (os.path.isfile(pdf_xlsx_omni) and os.path.isfile(pdf_txt_omni)):
						...
						while os.path.isfile(s_omni_pdf_filesave):
		...
		with open(test_error_log,'a') as er:
			er.write('Error in internal Omni Conversion'+'|'+Request_id+'|'+str(e)+'|'+str(datetime.now())+'\n')

		omni_error_convert  = (glob.glob(omni_savedir+'/'+request_id+'*'))
		for e in omni_error_convert:
			os.remove(e)


if "External" in type_p:
...
# 	print str(e)
# 	with open(test_error_log,'a') as er:
# 		er.write('Error in External omni Conversion'+Request_id+'|'+str(e)+'|'+str(datetime.now())+'\n')
